...
Issue #1's snapshot above is historical. The user subsequently authorized issue #3 for branch automation, issue association checks, actionlint validation, and setup documentation. Its bootstrap branch is `3-add-issue-to-branch-automation-and-issue-association-checks`; preserve that existing name. See [issue workflow setup and verification](issue-workflow.md) for the selected settings and activation steps. Application code and deployment workflows remain excluded from issue #3.
...
The user manually creates every issue and chooses its scope. The approved automation has a separate responsibility: only `fabiomoggi` applying `ready-for-development` to an existing issue triggers creation of a linked working branch from current `main`. Opening an issue alone does not create a branch, and marking it ready does not itself authorize Codex implementation. Preserve the issue/branch/PR association check, repeat-trigger protection, and other technical automation decisions in the [CI/CD foundation](ci-cd-foundation.md).

Before that automation exists, GitHub's native issue-linked branch mechanism supplies the branch for a user-created, explicitly authorized setup issue. The readiness automation can be exercised after installation on the default branch through the normal reviewed process. Issue #3 settled the settings: only `fabiomoggi`, label `ready-for-development`, and names `<issue-number>-<title_slug>` with underscores replacing spaces and punctuation. Native bootstrap branches already created retain their existing names. The user subsequently selected naming-based branch association: correctly named manual branches qualify when their PR links to the matching issue. Do not require branch-origin artifacts or ancestry verification.
